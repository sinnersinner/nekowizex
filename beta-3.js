...
// mrank
aM = {"C":1, "C+":0.7, "B":0.63, "B+":0.6, "A":0.58, "A+":0.65, "S":0.7, "S+":0.7371, "SS":0.7667 , "L":0.6272};

aLv = {"C":10, "C+":20, "B":30, "B+":40, "A":50, "A+":60, "S":70, "S+":80, "SS":90, "L":110};
...
function normalmode(){
	...
	if ( document.getElementById("non1").checked ) {
		ncolor = 1;
		bn=(Math.round(bs));
		bm =Math.ceil(bs*1.25*aM[arank]) ;
	}
	else{
		
		bn=(Math.floor(bs/1.25));	//轉為初始值lv
		bm =Math.ceil(bs*aM[arank]) ;
	}
