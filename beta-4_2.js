...
//date:160717-想把程式碼砍掉重練... 太亂了 XD
//以及異色部分因bug暫時拿掉

// mrank
aM = {"C":1, "C+":0.7, "B":(19/30), "B+":0.6, "A":(29/50), "A+":(39/60), "S":(49/70), "S+":(59/80), "SS":(69/90), "SS+":(69/100), "L":(69/110)};

aLv = {"C":10, "C+":20, "B":30, "B+":40, "A":50, "A+":60, "S":70, "S+":80, "SS":90, "SS+":100, "L":100};
...
function DisplayFunc() {
	...
	arank = document.getElementById("a").value;	 //Input rank
	bs = document.getElementById("b").value;	//Input exp
	lv1= Number(bs);
	cs = document.getElementById("c").value;	//Input Lv
	csforcalc = document.getElementById("c").value;
	calcmode = document.getElementById("calc").value; 	//選擇模式
	maxlv = aLv[arank];
	...
	if(sth==1){
		...
		document.getElementById("state").value = " 需餵食經驗已扣掉該卡輸入等級,詳閱說明..."+"該卡初級提供同色卡經驗 "+lv1 +" exp";
	...
	//to test
	document.getElementById("2lv").value = lv;
	document.getElementById("2max").value = maxlv;
	document.getElementById("2mb").value = bm;
	document.getElementById("2b").value = bs;
	document.getElementById("2nb").value = bn;
	document.getElementById("2list").value = nextlist[lv];
	document.getElementById("ch").value = sth;
	document.getElementById("inputLv").value = cs;
	document.getElementById("test").value = lv1;
//to test
