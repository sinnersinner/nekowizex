...
// mrank
aM = {"C":1, "C+":(14/20), "B":(19/30), "B+":(24/40), "A":(29/50), "A+":(39/60), "S":(49/70), "S+":(59/80), "SS":(69/90), "L":(69/110)};
...
function DisplayFunc() {
	...
	if(cs == 1)
		...
		if(cs<1){
			...
			inputerror();
			document.getElementById("state").value = " 輸入錯誤，請重新輸入等級";
			return;
	...
	document.getElementById("zeat").value =  " "+(totallist[lv+1] - totallist[cs-1])+" exp";
	...
	if(sth==1){
		...
		if(cs>lv)
		{
			document.getElementById("state").value = " 該卡等級已達或超過最佳等級..."+"該卡初級提供同色卡經驗 "+bsetmode +" exp";
			document.getElementById("state").style.color = "red";
		}
